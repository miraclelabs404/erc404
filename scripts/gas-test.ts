...
async function main() {
  ...
  const miracleFactory = await ethers.getContractFactory("Miracle")
  const miracleContract = await miracleFactory.deploy(signers[0].address)
  await miracleContract.waitForDeployment()

  message("miracleContract")
  await runTests(miracleContract)
